...
public:
    ...
    CLightSource(const CLightSource&) = default;
...
public:
    ...
    CDirectionalLight(const CDirectionalLight& other) = default;
...
public:
    ...
    CPointLight(const CPointLight& other) = default;
...
void RegisterLightsBindings()
{
    ...
    r = engine->RegisterObjectBehaviour("DirectionalLight", asBEHAVE_CONSTRUCT, "void f(DirectionalLight &in)", asFUNCTION(CDirectionalLight::CreateCopy), asCALL_CDECL_OBJFIRST); assert(r >= 0);
    ...
    r = engine->RegisterObjectMethod("DirectionalLight", "DirectionalLight &opAssign(DirectionalLight &in)", asMETHODPR(CDirectionalLight, operator=, (const CDirectionalLight&), CDirectionalLight&), asCALL_THISCALL); assert(r >= 0);
    ...
    r = engine->RegisterObjectBehaviour("PointLight", asBEHAVE_CONSTRUCT, "void f(PointLight &in)", asFUNCTION(CPointLight::CreateCopy), asCALL_CDECL_OBJFIRST); assert(r >= 0);
    ...
    r = engine->RegisterObjectMethod("PointLight", "PointLight &opAssign(PointLight &in)", asMETHODPR(CPointLight, operator=, (const CPointLight&), CPointLight&), asCALL_THISCALL); assert(r >= 0);
